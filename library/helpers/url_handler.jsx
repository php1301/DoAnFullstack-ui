...
export function setStateToUrl(state) {
  ...
  Object.keys(state).forEach((key) => {
    //   dùng call tránh violate esLint, bỏ call để hiểu thêm
    // if (state.hasOwnProperty.call(key, state)) {
    ...
    switch (key) {
      ...
      case 'maxPrice':
        ...
        break;
      case 'amenities':
        urlData[key] = state[key] && typeof state[key] !== 'string' && state[key].length
          ? state[key].join()
          : state[key];
        break;
      case 'room':
